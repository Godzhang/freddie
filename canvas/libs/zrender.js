  ...
  ***************************************************************************** */
  ...
  var Track = (function () {
    ...
    Track.prototype.addKeyframe = function (time, value) {
      ...
      if (this.interpolable) {
        if (isArrayLike(value)) {
          ...
          if (
            (arrayDim === 1 && typeof value[0] !== "number") ||
    ...
    Track.prototype.step = function (target, percent) {
      ...
      if (this.useSpline) {
        ...
        var p2 =
          keyframes[frameIdx > kfsNum - 2 ? kfsNum - 1 : frameIdx + 1][
            valueKey
          ];
        var p3 =
          keyframes[frameIdx > kfsNum - 3 ? kfsNum - 1 : frameIdx + 2][
            valueKey
          ];
        if (arrDim > 0) {
          arrDim === 1
  ...
  var Element = (function () {
      ...
      if (textEl && (!textEl.ignore || forceUpdate)) {
        ...
        if (textConfig.position != null) {
          ...
          if (textOrigin && textConfig.rotation != null) {
            ...
            innerOrigin = true;
            attachedTransform.originX =
              -attachedTransform.x + relOriginX + (isLocal ? 0 : layoutRect.x);
            attachedTransform.originY =
              -attachedTransform.y + relOriginY + (isLocal ? 0 : layoutRect.y);
          }
        }
    ...
    Element.prototype.useStates = function (states, noAnimation) {
      if (!states.length) {
        ...
        var useHoverLayer = !!(
          stateObjects[len - 1] && stateObjects[len - 1].hoverLayer
        );
        if (useHoverLayer) {
          this._toggleHoverLayerFlag(true);
        }
    ...
    if (keyLen > 0 || (cfg.force && !animators.length)) {
      ...
      if (!additive && existsAnimatorsOnSameTarget.length) {
        for (var i = 0; i < existsAnimatorsOnSameTarget.length; i++) {
          var allAborted = existsAnimatorsOnSameTarget[i].stopTracks(
            changedKeys
          );
          if (allAborted) {
      ...
      var animator = new Animator(
        source,
        false,
        additive ? existsAnimatorsOnSameTarget : null
      );
  ...
  var Path = (function (_super) {
    ...
    Path.initDefaultProps = (function () {
      ...
      pathProto.__dirty =
        Element.REDARAW_BIT |
        Displayable.STYLE_CHANGED_BIT |
        Path.SHAPE_CHANGED_BIT;
    })();
    return Path;
  })(Displayable);
  ...
  function parseAttributes(xmlNode, el, defs, onlyInlineStyle) {
    ...
    each(
      ...
      function (propName) {
        zrStyle[propName] != null &&
          (disp.style[propName] = parseFloat(zrStyle[propName]));
      }
    );
    ...
    if (zrStyle.lineDash) {
      disp.style.lineDash = map(
        trim(zrStyle.lineDash).split(DILIMITER_REG),
        function (str) {
  ...
  function buildPath$2(ctx, shape) {
    ...
    if (!(radius > e)) {
      ...
      ctx.moveTo(
        x + radius * mathCos$3(startAngle),
        y + radius * mathSin$3(startAngle)
      );
      ctx.arc(x, y, radius, startAngle, endAngle, !clockwise);
      if (innerRadius > e) {
        ctx.moveTo(
          x + innerRadius * mathCos$3(endAngle),
          y + innerRadius * mathSin$3(endAngle)
        );
          ...
          if (it_1) {
            ...
            var a =
              ...
              mathSin$3(
                mathACos(
                  (x0 * x1 + y0 * y1) /
                    (mathSqrt$4(x0 * x0 + y0 * y0) *
                      mathSqrt$4(x1 * x1 + y1 * y1))
                ) / 2
              );
      ...
      if (!(arc > e)) {
        ...
        if (cr1 < cr) {
          ctx.arc(
            ...
            y + ct0.cy,
            cr1,
            mathATan2(ct0.y01, ct0.x01),
            mathATan2(ct1.y01, ct1.x01),
            !clockwise
          );
        } else {
          ctx.arc(
          ...
          ctx.arc(
            ...
            y,
            radius,
            mathATan2(ct0.cy + ct0.y11, ct0.cx + ct0.x11),
            mathATan2(ct1.cy + ct1.y11, ct1.cx + ct1.x11),
            !clockwise
          );
          ctx.arc(
            x + ct1.cx,
  ...
  var OrientedBoundingRect = (function () {
    ...
    OrientedBoundingRect.prototype.intersect = function (other, mtv) {
      ...
      maxTv$1.set(0, 0);
      if (
        !this._intersectCheckOneSide(this, other, minTv$1, maxTv$1, noMtv, 1)
      ) {
        overlapped = false;
        if (noMtv) {
          return overlapped;
      ...
      if (
        !this._intersectCheckOneSide(other, this, minTv$1, maxTv$1, noMtv, -1)
      ) {
        overlapped = false;
        if (noMtv) {
          return overlapped;
  ...
  function brushPath(ctx, el, style, inBatch) {
    ...
    if (!inBatch) {
      ...
      if (hasFillPattern) {
        fillPattern =
          el.__dirty || !el.__canvasFillPattern
      ...
      if (hasStrokePattern) {
        strokePattern =
          el.__dirty || !el.__canvasStrokePattern
    ...
    if (
      firstDraw ||
      el.__dirty & Path.SHAPE_CHANGED_BIT ||
  ...
  function bindCommonProps(ctx, style, prevStyle, forceSetAll, scope) {
    ...
    if (forceSetAll || style.opacity !== prevStyle.opacity) {
      ...
      ctx.globalAlpha =
        style.opacity == null ? DEFAULT_COMMON_STYLE.opacity : style.opacity;
    }
    ...
    Layer.prototype.createRepaintRects = function (
      ...
      for (var i = this.__startIndex; i < this.__endIndex; ++i) {
        ...
        if (el) {
          ...
          var prevRect =
            el.__isRendered &&
            (el.__dirty & Element.REDARAW_BIT || !shouldPaint)
              ? el.getPrevPaintRect()
              : null;
          if (prevRect) {
          ...
          var curRect =
            shouldPaint &&
            (el.__dirty & Element.REDARAW_BIT || !el.__isRendered)
              ? el.getPaintRect()
              : null;
          if (curRect) {
  ...
  function extractCommon(basePath, newArr, oldArr, diagonalPath, equals) {
    ...
    while (
      newPos + 1 < newLen &&
      oldPos + 1 < oldLen &&
      equals(newArr[newPos + 1], oldArr[oldPos + 1])
    ) {
      newPos++;
      oldPos++;
      commonCount++;
  ...
  function bindStyle(svgEl, style, el) {
    ...
    if (pathHasStroke(style)) {
      ...
      attr(
        svgEl,
        "stroke-width",
        (strokeScale_1 ? strokeWidth / strokeScale_1 : 0) + ""
      );
  ...
  var Definable = (function () {
    ...
    Definable.prototype.getDefs = function (isForceCreating) {
      ...
      if (defs.length === 0) {
        if (isForceCreating) {
          var defs_1 = svgRoot.insertBefore(
  ...
  var PatternManager = (function (_super) {
    ...
    function PatternManager(zrId, svgRoot) {
      return (
        _super.call(this, zrId, svgRoot, ["pattern"], "__pattern_in_use__") ||
        this
      );
    }
    PatternManager.prototype.addWithoutUpdate = function (
      svgElement,
      displayable
    ) {
      if (displayable && displayable.style) {
      ...
      if (displayable.style) {
        if (isPattern(displayable.style.fill)) {
          _super.prototype.markDomUsed.call(
            this,
            patternDomMap.get(displayable.style.fill)
          );
        }
        if (isPattern(displayable.style.stroke)) {
          _super.prototype.markDomUsed.call(
            this,
            patternDomMap.get(displayable.style.stroke)
          );
        }
      }
    };
  ...
  var ClippathManager = (function (_super) {
    ...
    function ClippathManager(zrId, svgRoot) {
      var _this =
        _super.call(this, zrId, svgRoot, "clipPath", "__clippath_in_use__") ||
        this;
      _this._refGroups = {};
    ...
    ClippathManager.prototype._getClipPathGroup = function (
      displayable,
      prevDisplayable
    ) {
      ...
      if (
        isClipPathChanged(
          clipPaths,
          prevDisplayable && prevDisplayable.__clipPaths
        )
      ) {
        keyDuplicateCount[clipPathKey] = keyDuplicateCount[clipPathKey] || 0;
        keyDuplicateCount[clipPathKey] &&
          (clipPathKey += "-" + keyDuplicateCount[clipPathKey]);
        keyDuplicateCount[clipPathKey]++;
      }
      return (
        this._refGroups[clipPathKey] ||
  ...
  var ShadowManager = (function (_super) {
    ...
    ShadowManager.prototype.update = function (svgElement, displayable) {
      ...
      if (hasShadow(style)) {
        ...
        var shadowDom = (displayable._shadowDom = this._shadowDomMap[
          shadowKey
        ]);
        if (!shadowDom) {
    ...
    ShadowManager.prototype.updateDom = function (
      svgElement,
      displayable,
      shadowDom
    ) {
  ...
  var SVGPainter = (function () {
    ...
    SVGPainter.prototype.toDataURL = function () {
      ...
      var outerHTML =
        svgDom.outerHTML || (svgDom.parentNode && svgDom.parentNode).innerHTML;
